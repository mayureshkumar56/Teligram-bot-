...
BOT_TOKEN = "8648804848:AAGsRKOF0SxYjZ1JUIqWXyDKAu0Mm7whiKE"
...
# ==========================================
# 6. VIDEO EXTRACTOR & TRIMMING (20 SECONDS FIX)
# ==========================================
async def download_and_trim_video(url: str, output_filename: str) -> bool:
    """Render Server ke liye optimized 20-second clip cutter"""
    try:
        user_agents = [
        ...
        try:
            stdout, _ = await asyncio.wait_for(process.communicate(), timeout=7.0)
        ...
        # 🌟 CHANGES HERE: Duration ko '-t 5' se badhakar '-t 20' (20 seconds) kar diya hai.
        # Server crash se bachne ke liye Single-Thread optimization barkarar hai.
        ffmpeg_cmd = [
        ...
        try:
            # 20 second ki badi file hai isliye timeout badhakar 15 second kiya hai
...
async def process_automated_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await update.message.reply_text("🔍 Video-Clip (20 Sec) Strict Filter chalu ho raha hai...")
    ...
    for link in links:
        ...
        try:
            # 🌟 CHANGES HERE: Purani description ("📢 New Post") hata di hai.
            # Ab sirf main link aur Join us link dikhega ekdum saaf!
            if join_link:
            ...
            # 20 second ki clips ke liye Render server ko stable rakhne ke liye 7 second ka delay
    ...
    await update.message.reply_text("🎉 All tasks finished successfully with 20s clips!", parse_mode="HTML")
...
async def run_bot():
    ...
    app.add_handler(CommandHandler("start", lambda u, c: u.message.reply_text("👋 Video (20s) Bot Active!")))
